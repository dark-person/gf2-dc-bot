...
// Core function to check if given time is within range of gun-smoke frontline.
func (c *Calendar) IsIntelligenceSupplies(t time.Time) (bool, error) {
	row, err := c.db.QueryRow("SELECT COUNT(*) FROM `calendar_ranged` WHERE start_at <= ? AND end_at >= ? AND schedule_name=? ORDER BY start_at", ConvertTimeToInt(t), ConvertTimeToInt(t), "情報補給")
