...
func (s *Scheduler) calcNextRangedDate() {
	...
	row, err := s.db.QueryRow("SELECT start_at, end_at FROM `calendar_ranged` WHERE schedule_name = '塵煙' AND is_predicted = 0")
	if err != nil {
	...
	for i := 0; i < 3; i++ {
		...
		_, err = s.db.Exec("INSERT OR IGNORE INTO `calendar_ranged` (schedule_name, start_at, end_at, is_predicted) VALUES ('塵煙', ?, ?, 1)",
